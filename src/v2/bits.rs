...
fn size() {
    ...
    expect!["2"].assert_eq(&estimated_bits!(Bits::<4>::try_from(2u8).unwrap()));
    expect!["3"].assert_eq(&estimated_bits!(Bits::<8>::try_from(7u8).unwrap()));
    expect!["3"].assert_eq(&estimated_bits!(Bits::<8>::try_from(6u8).unwrap()));
}
