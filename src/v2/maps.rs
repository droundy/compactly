...
impl<K: Encode + Hash + Eq, V: Encode> Encode for HashMap<K, V> {
    ...
    async fn decode_awaiting<D: super::AsyncEntropyDecoder>(
        ...
        for _ in 0..len {
            ...
            let k = <K as Encode>::decode_async(reader, &mut ctx.key).await?;
            let v = <V as Encode>::decode_async(reader, &mut ctx.value).await?;
            map.insert(k, v);
        }
        Ok(map)
    }
...
impl<K: Ord + Encode<SK>, SK, V: Encode<SV>, SV> Encode<Mapping<SK, SV>> for BTreeMap<K, V> {
    ...
    async fn decode_awaiting<D: super::AsyncEntropyDecoder>(
        ...
        for _ in 0..len {
            sentinel.decode_async(reader).await?;
            let k = <K as Encode<SK>>::decode_async(reader, &mut ctx.key).await?;
            let v = <V as Encode<SV>>::decode_async(reader, &mut ctx.value).await?;
            pairs.push((k, v));
        }
        Ok(pairs.into_iter().collect())
    }
