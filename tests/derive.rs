...
/// The derive computes `MAX_BYTES` by summing field bounds within a variant and
/// taking the max across variants, on top of the discriminant. Worth pinning
/// down: nobody reviews a generated constant, and an understated bound would let
/// the async decoder read past the end of its buffer and return plausible
/// garbage.
mod max_bytes {
    use compactly::v2::{AtMost, Encode};

    #[derive(compactly::Encode)]
    struct Fields {
        flag: bool,
        byte: u8,
        wide: u64,
    ...
    fn a_struct_sums_its_fields() {
        ...
        assert_eq!(<AtMost<0> as Encode>::MAX_BYTES, 0);
        assert_eq!(
            <Fields as Encode>::MAX_BYTES,
            <bool as Encode>::MAX_BYTES + <u8 as Encode>::MAX_BYTES + <u64 as Encode>::MAX_BYTES
        );
    }
    ...
    fn an_enum_maxes_over_variants_atop_the_discriminant() {
        ...
        assert_eq!(
            <Variants as Encode>::MAX_BYTES,
            <AtMost<2> as Encode>::MAX_BYTES
                + 2 * <u64 as Encode>::MAX_BYTES
    ...
    fn a_field_uses_its_own_strategys_bound() {
        use compactly::v2::EncodingStrategy;
        use compactly::Small;
        assert_eq!(
            <WithStrategy as Encode>::MAX_BYTES,
            <Small as EncodingStrategy<u64>>::MAX_BYTES + <u64 as Encode>::MAX_BYTES
        );
    }
    ...
    fn one_unbounded_field_makes_the_whole_type_unbounded() {
        ...
        assert_eq!(<String as Encode>::MAX_BYTES, usize::MAX);
        assert_eq!(<HasUnbounded as Encode>::MAX_BYTES, usize::MAX);
    }
    ...
    fn encoded_values_fit_within_the_computed_bound() {
        ...
        for wide in [0, 1, u64::MAX, 1 << 31] {
            for flag in [false, true] {
                ...
                assert!(
                    n <= <Fields as Encode>::MAX_BYTES + SETTLING,
